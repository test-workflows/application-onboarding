...
:DEFAULT_REGISTRY
    ...
    SET REGISTRY_NAMESPACE=project2
	GOTO MAIN
...
echo "building and pushing image application-onboarding-buildstage"
pushd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/application-onboarding-buildstage .
popd

echo "building and pushing image application-onboarding"
pushd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/application-onboarding .
popd
