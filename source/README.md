...
## Running application-onboarding in Quarkus dev mode
...
```
(application-onboarding should be included in the returned array)
...
[application-onboarding-schema.json](./src/main/resources/schemas/application-onboarding-schema.json ), run the following to create an
instance of the application-onboarding workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/application-onboarding -d@input.json
```
